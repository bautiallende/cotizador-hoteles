...
// Obtiene valores de una hoja completa (columnas A–Z sin límite de filas)
...
// Lee y normaliza reglas de precio, incluyendo la ciudad
  ...
  const rules = rows
    .map((r) => {
      ...
      const basePrice = parseFloat(
        r.basePrice.replace(/[^0-9,-]/g, '').replace(',', '.')
      );
      return {
        hotelId: r.hotelId,
        city: r.city,
        ...
        roomCode: r.roomCode,
        capacity: {
          adults: parseInt(r.adults, 10),
          children: parseInt(r.children, 10),
          childAgeRanges: r.childAgeRanges
            .split(';')
            .map((s) => s.trim()),
        },
        startDate,
    ...
    .filter(
      (r) => r.startDate && r.endDate && !isNaN(r.basePrice)
    );
...
// Combina reglas por hotel e incluye city
export async function getHotelsData() {
  ...
  for (const pr of priceRules) {
    const { hotelId, city, roomCode, category, capacity, startDate, endDate, basePrice } = pr;
    if (!map[hotelId])
      map[hotelId] = { hotelId, city, roomsMap: {}, discounts: [] };
    map[hotelId].discounts = discountRules.filter(
      (d) => d.hotelId === hotelId
    );
    const key = `${roomCode}|${category}`;
    if (!map[hotelId].roomsMap[key]) {
      map[hotelId].roomsMap[key] = {
        roomCode,
        category,
        capacity,
        priceRules: [],
  ...
  return Object.values(map).map((h) => ({
    hotelId: h.hotelId,
    city: h.city,
    rooms: Object.values(h.roomsMap),
